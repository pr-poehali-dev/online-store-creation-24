...
const categories = [
  { title: 'Комплекты мебели', icon: 'LayoutGrid' },
  { title: 'Тумбы с раковиной', icon: 'Archive' },
  { title: 'Тумбы без раковины', icon: 'Box' },
  { title: 'Зеркала', icon: 'Square' },
  { title: 'Зеркальные шкафы', icon: 'PanelTop' },
  { title: 'Пеналы', icon: 'RectangleVertical' },
  { title: 'Подвесные шкафы', icon: 'Frame' },
  { title: 'Полки', icon: 'AlignJustify' },
  { title: 'Комоды', icon: 'Package' },
];

const products = [
  { name: 'Antonio Lupi Sesamo', type: 'Тумба с раковиной', price: '189 990 ₽', cat: 'Тумбы с раковиной', icon: 'Archive' },
  { name: 'Ideal Standard Adapto', type: 'Комплект мебели', price: '142 000 ₽', cat: 'Комплекты мебели', icon: 'LayoutGrid' },
  { name: 'Roca Beyond', type: 'Тумба без раковины', price: '76 400 ₽', cat: 'Тумбы без раковины', icon: 'Box' },
  { name: 'Villeroy & Boch More', type: 'Зеркало с подсветкой', price: '54 900 ₽', cat: 'Зеркала', icon: 'Square' },
  { name: 'Cersanit Smart', type: 'Зеркальный шкаф', price: '38 200 ₽', cat: 'Зеркальные шкафы', icon: 'PanelTop' },
  { name: 'Aquaton Форли', type: 'Пенал', price: '22 100 ₽', cat: 'Пеналы', icon: 'RectangleVertical' },
  { name: 'Jacob Delafon Odeon', type: 'Подвесной шкаф', price: '29 700 ₽', cat: 'Подвесные шкафы', icon: 'Frame' },
  { name: 'Antonio Lupi Fold', type: 'Полка', price: '9 400 ₽', cat: 'Полки', icon: 'AlignJustify' },
  { name: 'Salini Storage', type: 'Комод', price: '64 500 ₽', cat: 'Комоды', icon: 'Package' },
];

const filters = [
  { title: 'Цена', items: ['до 30 000 ₽', '30 000 – 80 000 ₽', '80 000 – 150 000 ₽', 'от 150 000 ₽'] },
  { title: 'Бренд', items: ['Antonio Lupi', 'Villeroy & Boch', 'Roca', 'Ideal Standard', 'Cersanit'] },
  { title: 'Материал', items: ['МДФ', 'Массив дерева', 'Стекло', 'Керамика'] },
  { title: 'Цвет', items: ['Белый', 'Дуб', 'Венге', 'Чёрный'] },
];

const Furniture = () => {
  const [activeCat, setActiveCat] = useState('Тумбы с раковиной');
    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-[hsl(var(--gold))] selection:text-[hsl(var(--navy-deep))]">
      ...
      <div className="border-b border-border/60 bg-[hsl(var(--navy))]">
        <div className="container flex items-center gap-2 py-3 text-xs text-muted-foreground">
          ...
          <span className="text-foreground">Мебель для ванной</span>
          <Icon name="ChevronRight" size={13} />
      ...
      <section className="container pt-10 pb-6">
        ...
        <h1 className="font-display silver-text text-4xl md:text-5xl tracking-[0.1em] font-bold">МЕБЕЛЬ ДЛЯ ВАННОЙ</h1>
